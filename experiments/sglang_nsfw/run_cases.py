#!/usr/bin/env python3
"""Run sampled MiniMax-H3 NSFW cases against a local SGLang server."""
...
ROOT = Path("/mnt/luca/H3_infer/ComfyUI-master_cp/output/exp_sglang_nsfw")
...
def run_case(base: str, case: dict) -> dict:
    ...
    print(
        f"POST {case['id']} task={case['task']} steps={payload['num_inference_steps']} "
        f"ratio={case['target']['aspect_ratio']}",
        flush=True,
    )
    t0 = time.time()
    try:
...
def main() -> None:
    ...
    for case in cases:
        try:
            results.append(run_case(args.base, case))
        except Exception as e:
            rec = {"id": case["id"], "status": "exception", "error": repr(e)}
            print(f"exception {case['id']}: {e}", flush=True)
            results.append(rec)
            (ROOT / case["mode"] / f"{case['id']}.json").write_text(
                json.dumps(rec, ensure_ascii=False, indent=2) + "\n"
            )

    summary = ROOT / f"summary_{args.server}.json"
    ok = sum(1 for r in results if r.get("status") in {"ok", "skipped"})
    summary.write_text(
        json.dumps(
            {"server": args.server, "ok": ok, "n": len(results), "results": results},
            ensure_ascii=False,
            indent=2,
        )
        + "\n"
    )
    print(f"done {args.server} ok={ok}/{len(results)} summary={summary}", flush=True)
    if ok == 0:
        sys.exit(2)
