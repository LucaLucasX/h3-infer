#!/usr/bin/env python3
"""T2V batch with Kijai w4a8_mixed DiT on L0 stack (sage2 + Sparge 0.5 + 4-step turbo LoRA)."""
...
UNET_W4A8 = "minimax_h3_fl2va_pruned_w4a8_mixed.safetensors"
VAE_FP16 = "minimax_h3_video_vae_fp16.safetensors"
...
def main() -> None:
    ...
    ap.add_argument("--out-root", default="exp_w4a8_dit")
    args = ap.parse_args()
    ...
    print(
        f"w4a8_dit t2v: unet={UNET_W4A8} vae={VAE_FP16} {WIDTH}x{HEIGHT} "
    ...
    if not args.skip_warmup:
        ...
        w = run_one(args.comfy, graph_for(args.out_root, "_warmup", cases[0]["prompt"], length=length), "_warmup")
    ...
    for c in cases:
        ...
        r = run_one(args.comfy, graph_for(args.out_root, label, c["prompt"], length=length), label)
