...
try:
    ...
    if _SPAS_OK:
        ...
        def _sparge_audio_boosted(q, k, v, tensor_layout: str, topk: float, kwargs: dict):
            ...
            if not ranges:
                # Without h3_pack_segments (model.py), audio_dense silently degrades to full topk.
                if not getattr(_sparge_audio_boosted, "_warned_no_ranges", False):
                    logging.warning(
                        "H3-SpargeAttn-Exp: audio_dense=True but no audio ranges "
                        "(missing h3_pack_segments?) — falling back to full Sparge topk"
                    )
                    _sparge_audio_boosted._warned_no_ranges = True
