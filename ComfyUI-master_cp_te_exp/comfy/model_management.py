...
class LoadedModel:
    ...
    def model_memory(self):
        return self.model.model_size() + self.model.managed_extra_memory_size(self.device)

    def model_loaded_memory(self):
        return self.model.loaded_size()

    def model_offloaded_memory(self):
        # Managed caches duplicate offloaded weights and are not base weights
        # that still need to be loaded.
        return max(0, self.model.model_size() - self.model.base_loaded_size())
    ...
    def model_unload(self, memory_to_free=None, unpatch_weights=True):
        if memory_to_free is not None:
            cache_freed = self.model.free_managed_extra_memory(memory_to_free, self.device)
            memory_to_free -= cache_freed
            if memory_to_free <= 0:
                return False
            if memory_to_free < self.model.base_loaded_size():
                freed = self.model.partially_unload(self.model.offload_device, memory_to_free)
                if freed >= memory_to_free:
                    return False
        else:
            self.model.free_managed_extra_memory(device=self.device)
    ...
    for x in can_unload_sorted:
        ...
        if not DISABLE_SMART_MEMORY or device is None:
            ...
            if current_loaded_models[i].model.is_dynamic() and for_dynamic:
                ...
                #as VBAR works on-demand. Managed non-VBAR caches must be
                #explicitly evicted first; they cannot fault themselves out.
                if memory_to_free > 0:
                    current_loaded_models[i].model.free_managed_extra_memory(memory_to_free, device)
