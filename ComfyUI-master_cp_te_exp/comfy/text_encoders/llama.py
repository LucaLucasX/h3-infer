...
from comfy.ldm.modules.attention import attention_pytorch, optimized_attention_for_device
...
class Llama2_(nn.Module):
    ...
    def forward(self, x, attention_mask=None, embeds=None, num_tokens=None, intermediate_output=None, final_layer_norm_intermediate=True,
        ...
        causal_sdpa = self.use_causal_sdpa and attention_mask is None and past_len == 0 and seq_len > 1
        if seq_len > 1 and not causal_sdpa:
        ...
        if causal_sdpa:
            optimized_attention = lambda q, k, v, heads, **kwargs: attention_pytorch(q, k, v, heads, is_causal=True, **kwargs)
        else:
