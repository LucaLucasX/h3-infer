...
class ModelPatcher:
    ...
    def managed_extra_memory_size(self, device=None):
        """CUDA caches owned by a model but not registered as parameters."""
        total = 0
        for module in self.model.modules():
            size_fn = getattr(module, "comfy_managed_cuda_cache_size", None)
            if size_fn is not None:
                total += int(size_fn(device=device))
        return total

    def free_managed_extra_memory(self, memory_to_free=1e32, device=None):
        freed = 0
        for module in self.model.modules():
            free_fn = getattr(module, "comfy_evict_managed_cuda_cache", None)
            if free_fn is not None:
                freed += int(free_fn(memory_to_free=max(0, memory_to_free - freed), device=device))
            if freed >= memory_to_free:
                break
        return freed

    def base_loaded_size(self):
        return self.model.model_loaded_weight_memory

    def loaded_size(self):
        return self.base_loaded_size() + self.managed_extra_memory_size(self.load_device)
    ...
    def detach(self, unpatch_all=True):
        self.eject_model()
        self.free_managed_extra_memory(device=self.load_device)
