...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def process_tokens(self, tokens, device):
        ...
        for x in tokens:
            ...
            tokens_embed = self.transformer.get_input_embeddings()(tokens_embed, out_dtype=self.embedding_dtype)
            batched_embeds = None
            if other_embeds and hasattr(self.transformer, "preprocess_embeds"):
                batched_embeds = self.transformer.preprocess_embeds(
                    [item[1] for item in other_embeds], device=device
                )
                if len(batched_embeds) != len(other_embeds):
                    raise ValueError("preprocess_embeds must return one result per input embed")
            index = 0
            pad_extra = 0
            embeds_info = []
            for embed_num, o in enumerate(other_embeds):
                emb = o[1]
                ...
                batch_result = None if batched_embeds is None else batched_embeds[embed_num]
                if batch_result is not None:
                    emb, extra = batch_result
                ...
                ind = index + o[0]
                emb = emb.view(1, -1, emb.shape[-1]).to(device=device, dtype=self.embedding_dtype)
            ...
            if pad_extra > 0:
                padd_embed = self.transformer.get_input_embeddings()(torch.tensor([[self.special_tokens["pad"]] * pad_extra], device=device, dtype=torch.long), out_dtype=self.embedding_dtype)
    ...
    def forward(self, tokens):
        ...
        outputs = self.transformer(None, attention_mask_model, embeds=embeds, num_tokens=num_tokens, intermediate_output=intermediate_output, final_layer_norm_intermediate=self.layer_norm_hidden_state, dtype=self.embedding_dtype, embeds_info=embeds_info)
