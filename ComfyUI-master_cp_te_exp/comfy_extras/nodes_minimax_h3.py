...
CANVAS_MULTIPLE = 32
REF_IMAGE_SHORT_EDGE = 704
REF_IMAGE_SHORT_EDGE_MAX = 2048
REF_VIDEO_SHORT_EDGE = 544
FPS = 24
AUDIO_LATENT_FPS = 40
...
def scale_short_align_long(width, height, short, multiple=CANVAS_MULTIPLE):
    """Pin the short side, keep aspect, snap the long side to `multiple`.

    Short targets are 32-aligned (image 704, video 544) so VAE 16× latent is even
    for the DiT 2×2 patch. A 540p source is therefore 544, not 540.
    """
    target = short
    if width <= height:
        tw = target
        th = max(multiple, round(height * target / width / multiple) * multiple)
    else:
        th = target
        tw = max(multiple, round(width * target / height / multiple) * multiple)
...
def _short_edge_resize(image, short):
    h, w = image.shape[1], image.shape[2]
    tw, th = scale_short_align_long(w, h, short)
    if tw == w and th == h:
        return image[..., :3]
    return _resize(image, tw, th, "disabled")
...
def _encode_ref_audio(audio_vae, audio):
    ...
    waveform = waveform[:1].movedim(1, -1)
    comfy.model_management.synchronize()
    started = time.perf_counter()
    z = audio_vae.encode(waveform)  # [1, 32, 2, T]
    comfy.model_management.synchronize()
    print(f"[H3_VAE_TIMING] ref_audio_encode={time.perf_counter() - started:.3f}s samples={waveform.shape[-2]}", flush=True)
    return z, z.shape[-1]


def _encode_ref_vae(vae, pixels, kind):
    comfy.model_management.synchronize()
    started = time.perf_counter()
    z = vae.encode(pixels)
    comfy.model_management.synchronize()
    print(f"[H3_VAE_TIMING] {kind}={time.perf_counter() - started:.3f}s pixels={tuple(pixels.shape)}", flush=True)
    return z
...
class MiniMaxH3ImageToVideo(io.ComfyNode):
    ...
    def execute(cls, clip, vae, prompt, width, height, length,
        ...
        if first_frame is not None:
            img = _short_edge_resize(first_frame[:1], REF_IMAGE_SHORT_EDGE)
            img = _resize(img, width, height, "disabled")
            images.append(img)
            keyframes.append({"resolved_frame_index": 0, "image": img})
        if last_frame is not None:
            img = _short_edge_resize(last_frame[:1], REF_IMAGE_SHORT_EDGE)
            img = _resize(img, width, height, "center")
            images.append(img)
...
class MiniMaxH3AddGuide(io.ComfyNode):
    ...
    def execute(cls, positive, latent, frame_idx, vae=None, audio_vae=None, image=None, audio=None) -> io.NodeOutput:
        ...
        if image is not None:
            frames = _short_edge_resize(image[:guide_frames], REF_IMAGE_SHORT_EDGE)
...
class MiniMaxH3ReferenceToVideo(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input("ref_image_size", options=["match", "max"], default="match",
                    tooltip="Reference image short-edge. 'match' pins 704px; 'max' pins 2048px. Long side keeps aspect and snaps to 32. 704/2048 are already 32-aligned so VAE latents stay even for the DiT 2×2 patch. Reference tokens ride through every sampling step, so 'max' can be several times slower."),
                io.Autogrow.Input("ref_images", optional=True,
                    template=io.Autogrow.TemplatePrefix(
                        input=io.Image.Input("ref_image", tooltip="Reference image; match pins short edge to 704, max pins 2048"),
                        prefix="ref_image_", min=0, max=9)),
                io.Autogrow.Input("ref_videos", optional=True,
                    template=io.Autogrow.TemplatePrefix(
                        input=io.Image.Input("ref_video", tooltip="Reference video frames at 24 fps (2-15s); short edge pinned to 544, long side snaps to 32"),
    ...
    def execute(cls, clip, vae, audio_vae, prompt, width, height, length, ref_image_size="match",
        ...
        for img in (ref_images or {}).values():
            if img is None:
                continue
            short = REF_IMAGE_SHORT_EDGE if ref_image_size == "match" else REF_IMAGE_SHORT_EDGE_MAX
            resized = _short_edge_resize(img[:1], short)
            print(
                f"[H3_REF_IMAGE] input={img.shape[2]}x{img.shape[1]} resize={resized.shape[2]}x{resized.shape[1]} short={short}",
                flush=True,
            )
            z = _encode_ref_vae(vae, resized, "ref_image_encode")
            ref_items.append({"type": "image", "data": resized})
            ref_blocks.append({"kind": "image", "latent_h": int(z.shape[-2]), "latent_w": int(z.shape[-1]), "latent": z})
        ...
        for name, video_frames in (ref_videos or {}).items():
            ...
            frames = _short_edge_resize(video_frames, REF_VIDEO_SHORT_EDGE)
            ch, cw = frames.shape[1], frames.shape[2]
            ...
            while n % 17 != 5:
                n -= 1
            frames = frames[:n]
            sample_count = len(range(0, n, FPS // 2))
            print(
                f"[H3_REF_VIDEO] name={name} input_frames={video_frames.shape[0]} "
                f"resize={cw}x{ch} target_frames={frame_count} encoded_frames={n} "
                f"truncated={video_frames.shape[0] > frame_count} qwen_frames={sample_count}",
                flush=True,
            )
            z = _encode_ref_vae(vae, frames, "ref_video_encode")
            ...
            ref_blocks.append({"kind": "video_audio" if ref_audio_t else "video",
                               "latent_t": z.shape[2], "latent_h": int(z.shape[-2]), "latent_w": int(z.shape[-1]),
