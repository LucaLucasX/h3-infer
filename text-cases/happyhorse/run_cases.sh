...
if [ "$#" -lt 1 ]; then
  echo "usage: $0 <port> [tasklist] [outdir] [steps]" >&2
  exit 2
fi
...
mkdir -p "$OUT"
while IFS='|' read -r name duration aspect; do
  [ -z "$name" ] && continue
  ...
  "$PY" "$HERE/../../fl2va.py" \
    --prompt-file "$HERE/${name}.txt" \
    --duration "$duration" --aspect-ratio "$aspect" --steps "$STEPS" \
    --port "$PORT" --out-dir "$OUT" --name "${name}_s${STEPS}" \
    --poll-interval 10
done < "$LIST"
